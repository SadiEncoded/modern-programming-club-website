...
export const SiteFooter = () => (
  <footer className="relative bg-background border-t border-border overflow-hidden pt-12 pb-8 w-full">
    <div className="relative z-10 px-6 md:px-12 lg:px-24">
      
      {/* Top CTA Row */}
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 pb-12 mb-12 border-b border-border">
        <div className="space-y-1">
          <h4 className="text-2xl font-black text-foreground" style={{ fontFamily: "var(--font-orbitron), sans-serif" }}>
        ...
        <Link 
          href="/join"
          className="group flex items-center gap-3 px-8 py-3.5 bg-foreground text-background text-[10px] font-black uppercase tracking-[0.2em] rounded-full hover:bg-primary hover:text-white transition-all duration-300 shadow-xl dark:shadow-none"
          style={{ fontFamily: "var(--font-orbitron), sans-serif" }}
        >
          Join Tomorrow&apos;s Innovators <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 mb-16">
        ...
        <div className="lg:col-span-7 grid grid-cols-2 md:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            <h5 className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground" style={{ fontFamily: "var(--font-orbitron), sans-serif" }}>
              Explore
            </h5>
            <ul className="space-y-3">
              {NAVIGATION.footerLinks[0].links.map((link) => (
                <li key={link.label}>
                  <Link href={link.href} className="text-muted-foreground hover:text-primary transition-colors text-sm font-semibold">
                    {link.label}
                  </Link>
                </li>
              ))}
            ...
            <div className="flex flex-col gap-3">
              {[
                ...
                <a 
                  ...
                  className="flex items-center gap-2 text-muted-foreground hover:text-primary transition-colors text-sm font-semibold group"
                >
          ...
          <div className="space-y-6">
            ...
            <ul className="space-y-3">
              ...
              <li><Link href="/contact" className="text-muted-foreground hover:text-primary transition-colors text-sm font-semibold">Contact Us</Link></li>
            </ul>
          </div>
        </div>
      </div>
